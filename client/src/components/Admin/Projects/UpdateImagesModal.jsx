...
const UpdateImagesModal = ({ project, onClose }) => {
  ...
  const initialFiles =
    project.files?.map((f) => {
      const img = project.images?.find((i) => i.id == f.id);
      return {
  ...
  const handleSave = async () => {
    ...
    await saveFilesProject(project.id, localFiles, filesJson);
    onClose();
  };
